...
class AssetTransfer extends Contract {
    ...
    /// Create New User
    async CreateUser(ctx, name, address, imageUrl, mobileNo, emailId, publicId, status, password, createdAt) {
        ...
        try {
          await ctx.stub.putState(publicId, Buffer.from(stringify(sortKeysRecursive(user))));  
        } catch (e){
            console.log(e);
        }
    ...
    async ValidateUser(ctx, publicId, password) {
        const exists = await this.CheckPublicIdPasswordExist(ctx, publicId, password);
        if (exists == null) {
    ...
    async AddSkill(ctx, userId, skillId, skillDocUrl, instituteId) {
        ...
        await ctx.stub.putState(userId, Buffer.from(stringify(sortKeysRecursive(exists))));
        return exists;
    }
    ...
    async CheckEmailExist(ctx, email) {
        ...
        const assetJSON = await this.GetQueryResultForQueryString(ctx, JSON.stringify(queryString));
    ...
    async AddDummyUsers(ctx, count){
        try {
            for (let i = 1; i <= count; i++) {
               let res =  await this.CreateUser(ctx, stringify(i), "user name" + i, "address " + i, "imageUrl_"+i, "mobileNo_"+i, "emailId_"+i, "publicId_"+i, true, "123456", "2023-11-09T10:37:26.031Z")
            }
            return "Success";
        } catch (error) {
            return Error(error);
